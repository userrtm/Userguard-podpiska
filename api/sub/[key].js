export default async function handler(req, res) {
  ...
  const target = `https://pasarguard.lexum.sbs:8000/sub/${encodeURIComponent(key)}`;
  const subUrl = `https://gameus.sbs/api/sub/${encodeURIComponent(key)}`;
  ...
  if (isBrowser) {
    const username = decodeNameFromKey(key) || "Userguard UserrTM";
  ...
  try {
    ...
    res.setHeader("profile-title", "Userguard UserrTM");
...
.card:before{
  content:"VPN FAST SECURE Userguard USERRTM VPN FAST SECURE Userguard USERRTM";
  position:absolute;
  top:40%;
  left:-40%;
  width:180%;
...
<div class="bgText">
  ...
  <span>Userguard VPN • Userguard USERRTM</span>
  <span>SECURE CONNECTION • FAST SERVER</span>
  <span>USERRTM SERVERS • VPN • Userguard</span>
</div>
<div class="lines"></div>
...
function openHiddify(){ location.href = "hiddify://import/" + subUrl + "#Userguard"; }
function openV2RayTun(){ location.href = "v2raytun://import/" + subUrl; }
function openV2RayNG(){ location.href = "v2rayng://install-sub?url=" + encodeURIComponent(subUrl) + "&name=" + encodeURIComponent("Userguard UserrTM"); }
